...
void D_DoomLoop (void)
{
    ...
    DEH_printf("D_DoomLoop: Game loop started.\n");

    DEH_printf("D_DoomLoop: Running first tic.\n");
    TryRunTics();
    DEH_printf("D_DoomLoop: Ran first tic.\n");

    DEH_printf("I_SetWindowTitle: Setting window title.\n");
    I_SetWindowTitle(gamedescription);
    DEH_printf("I_SetWindowTitle: Set window title.\n");

    DEH_printf("I_GraphicsCheckCommandLine: Checking command line.\n");
...
//    W_PrintDirectory();
    ...
    ST_Init ();
    DEH_printf("ST_Init: Finished initializing status bar.\n");
    ...
    if (gamemode == commercial && W_CheckNumForName("map01") < 0) {
        DEH_printf("Game is store demo, likely Doom II.\n");
        storedemo = true;
    }
    ...
    if (startloadgame >= 0)
    {
        ...
        G_LoadGame(file);
        DEH_printf("G_LoadGame: Loaded savegame.\n");
    }

    if (gameaction != ga_loadgame )
    {
		if (autostart || netgame) {
            DEH_printf("G_InitNew: Game is autostart or a netgame.\n");
			G_InitNew (startskill, startepisode, startmap);
        } else {
            DEH_printf("D_StartTitle: Rendering title.\n");
			D_StartTitle ();                // start up intro loop
        }
    }

    DEH_printf("D_DoomLoop: Gameplay looped starting.\n");
    D_DoomLoop ();  // never returns
}
